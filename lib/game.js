(function() {
  ...
  Game.prototype.processShipCollisions = function() {
    var playerShipParams, currShipParams;
    playerShipParams = this.playerShip.collisionParams();
    for(var i = 0; i < this.enemyShips.length; i++) {
      var currShip = this.enemyShips[i];
      currShipParams = currShip.collisionParams();
      if (SpaceGame.Util.collision(playerShipParams, currShipParams)) {
        this.playerShip.hit(currShip.health);
        currShip.hit(currShip.health + 1);
      } 
    }
  };
})();
