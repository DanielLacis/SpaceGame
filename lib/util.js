(function() {
  ...
  Util.addVecs = function(position, velocity) {
    var x = position[0] + velocity[0];
    var y = position[1] + velocity[1];
    return [x, y];
  };

  Util.sinusoidPositionsDown = function (options) {
  ...
  Util.straightUp = function (options) {
    var nextPos;
    var startPos = options.startPos;
    var lastPos = startPos;
    var posArr = [lastPos];
  ...
  Util.horizontal = function (options) {
    var nextPos;
    var startPos = options.startPos;
    var lastPos = startPos;
    var posArr = [lastPos];
  ...
  Util.validPosition = function (pos, maxes) {
    return ((pos[1] >= 0 && pos[1] <= maxes[1]) && (pos[0] >= 0 && pos[0] <= maxes[0]));
  };

  Util.keepInBounds = function (pos, dims, offset) {
    for(var i = 0; i < 2; i++) {
      if (pos[i] < 0 + offset) {
        pos[i] = 0 + offset;
  ...
  Util.collision = function(options1, options2) {
